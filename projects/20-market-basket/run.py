...
def main() -> None:
    ...
    print(
        f"  {len(counts):,} distinct products; rules built on the top {TOP_ITEMS} "
    ...
    print(
        f"  {(rules['lift'] > 1).mean():.1%} of them have lift above 1, which the "
    ...
    print(
        f"    {len(survivors):,} of {len(rules):,} rules ({len(survivors) / len(rules):.1%}) "
        f"exceed it."
    )
    ...
    for label, ranked in (
        ("by lift", rules.sort_values("lift", ascending=False)),
        ...
        print(
            f"  top 50 {label:<14} {held:>6.0%} still have lift > 1   "
    ...
    print(
        f"  top 50 by lift, unbounded    {wide_check['held'].mean():>6.0%} "
    ...
    left.scatter(
        rules["consequent_support"], rules["confidence"], s=4, alpha=0.15, color=PALETTE["blue"]
    )
    ...
    right.text(
        rules["support_count"].max(),
        threshold * 1.1,
        f"max lift under reshuffling ({threshold:.0f})",
        fontsize=8,
        ha="right",
        color=PALETTE["red"],
        fontweight="bold",
    ...
    right.set_xlabel("Baskets supporting the rule")
    right.set_ylabel("Lift")
    right.set_title("and the biggest lifts rest on the fewest baskets")
    caption(
        fig,
        f"{len(rules):,} rules from {len(baskets):,} baskets. Both failure modes are "
    ...
    caption(
        fig,
        "Null: items reassigned to baskets at random, preserving basket sizes and "
        "item popularity. 12 replicates. Everything real structure contributes is "
        "destroyed; the marginals are not.",
    )
    print(
        "  " + str(save(fig, FIGURES / "02-permutation-null.png").relative_to(HERE.parent.parent))
    )
    ...
    for ax, check, title in (
        ...
        ax.set_xscale("log")
        ax.set_yscale("log")
        ax.set_xlabel("Lift when discovered")
        ax.set_ylabel("Lift the next quarter")
        ax.set_title(f"{title} — {check['held'].mean():.0%} still associated")
    caption(
        fig,
        "Dashed line: the rule holds exactly as measured. Points below it are rules "
    ...
    labels = [
        f"{shorten(r.antecedent, 22)}\n-> {shorten(r.consequent, 22)}" for r in best.itertuples()
    ]
    ...
    for bar, r in zip(bars, best.itertuples(), strict=True):
        ax.text(
            ...
            f"lift {r.lift:.1f}, {r.support_count} baskets",
            va="center",
            fontsize=7.5,
        )
    ...
    caption(
        fig,
        "Leverage is in baskets, not ratios. It cannot be inflated by a rare "
