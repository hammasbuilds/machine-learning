...
def main() -> None:
    ...
    fits = [
        fit_penalised(
            X_train, y_train, X_test, y_test, columns, name="Ridge (L2)", penalty="l2", C=C_CHOSEN
        ),
        fit_penalised(
            X_train, y_train, X_test, y_test, columns, name="Lasso (L1)", penalty="l1", C=C_CHOSEN
        ),
        fit_penalised(
            X_train,
            y_train,
            ...
            y_test,
            columns,
            name="Elastic net (0.5)",
            penalty="elasticnet",
            C=C_CHOSEN,
            l1_ratio=0.5,
        ),
        fit_penalised(
            X_train, y_train, X_test, y_test, columns, name="No penalty", penalty=None, C=1.0
        ),
    ]
    print(f"  {'model':<20}{'train AUC':>11}{'test AUC':>10}{'features kept':>16}")
    for fit in fits:
        print(
            f"  {fit.name:<20}{fit.train_auc:>11.4f}{fit.test_auc:>10.4f}"
    ...
    for _, row in per_strength.iloc[::3].iterrows():
        print(
            f"  {row['C']:>10.4f}{int(row['n_selected']):>7}{row['train_auc']:>12.4f}"
    ...
    for _, row in stability.iterrows():
        ...
        print(
            f"  {row['column']:<20}{row['selected_share']:>10.1%}"
    ...
    if len(unstable_sign):
        print(
            f"    {len(unstable_sign)} flip sign across resamples: "
    ...
    caption(
        fig,
        "Fourteen features engineered from five sensors. Every dark cell is a pair "
    ...
    fig, (top, bottom) = plt.subplots(
        2, 1, figsize=(9.5, 6.4), sharex=True, gridspec_kw={"height_ratios": [2, 1]}
    )
    ...
    top.set_xscale("log")
    top.axhline(0, color="#333333", lw=0.8)
    ...
    bottom.set_xlabel("C (higher = weaker penalty)")
    bottom.set_ylabel("ROC-AUC")
    bottom.legend(fontsize=8)
    bottom.set_title("and what it costs: almost nothing, over three orders of magnitude")
    caption(
        fig,
        f"AI4I 2020, 70/30 split. The score is flat from C={strengths[4]:.3f} upward "
    ...
    caption(
        fig,
        "Shaded band: selected between 25% and 75% of the time. A feature in that "
    ...
    right.barh(x + width / 2, unscaled.coefficients, width, label="unscaled", color=PALETTE["red"])
    right.set_yticks(x, columns, fontsize=7)
    right.axvline(0, color="#333333", lw=0.8)
    right.set_xlabel("Coefficient")
    right.legend(fontsize=8)
    right.set_title("Same penalty, different units")
    caption(
        fig,
        "Left: 780 pairs of bootstrap refits. Right: identical model and penalty "
        "strength, run before and after standardising.",
    )
    print(
        "  "
        + str(save(fig, FIGURES / "04-overlap-and-scaling.png").relative_to(HERE.parent.parent))
    )
        ...
        json.dumps(
            {
                ...
                "penalties": [
                    {
                        "model": f.name,
                        "train_auc": round(f.train_auc, 4),
                        "test_auc": round(f.test_auc, 4),
                        "kept": f.n_selected,
                    }
                    for f in fits
                ],
