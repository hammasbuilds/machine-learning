...
def main() -> None:
    ...
    for a in agreements:
        print(
            f"    {a.a:<22} vs {a.b:<22} overlap {a.overlap:>6.1%}   "
        ...
        rows.append(
            {
                ...
                "precision": p,
                "recall": r,
                "lift": lift_at_k(s, labels, k=BUDGET),
                "found": int(p * BUDGET),
            }
        )
    ...
    print(
        f"  {'random inspection':<24}{random_precision:>11.2%}{BUDGET / len(frame):>9.1%}"
    ...
    for mode in MODES:
        ...
        mode_rows.append(
            {"mode": mode, "total": present, "caught": caught, "share": caught / max(present, 1)}
        )
    ...
    curve_rows = [
        {
            "budget": b,
            "detector": name,
            "precision": precision_at_k(s, labels, k=b),
            "recall": recall_at_k(s, labels, k=b),
        }
        for b in budgets
        for name, s in scores.items()
    ]
    ...
    for (name, s), colour in zip(
        scores.items(), (PALETTE["blue"], PALETTE["orange"], PALETTE["green"]), strict=True
    ):
        right.hist(np.log1p(s - s.min()), bins=60, alpha=0.55, label=name, color=colour)
    right.set_xlabel("log anomaly score")
    right.set_ylabel("Cycles")
    right.legend(fontsize=8)
    right.set_title("Three score distributions, three shapes")
    fig.suptitle(
        "Before any label: do these methods even mean the same thing?",
        fontsize=12,
        fontweight="bold",
        x=0.02,
        ha="left",
    ...
    bars = ax.barh(
        [r["detector"] for r in order], [r["precision"] for r in order], color=PALETTE["blue"]
    )
    ax.axvline(random_precision, color=PALETTE["red"], lw=2, ls="--")
    ax.text(
        random_precision * 1.15,
        -0.42,
        "random inspection",
        color=PALETTE["red"],
        fontsize=9,
    ...
    caption(
        fig,
        f"Labels withheld from every detector and used only here. Base rate "
    ...
    bars = ax.bar(
        [m["mode"] for m in mode_rows], [m["share"] for m in mode_rows], color=colours, width=0.55
    )
    ...
    caption(
        fig,
        "TWF = tool wear, HDF = heat dissipation, PWF = power, OSF = overstrain, "
    ...
    left.set_xscale("log")
    left.set_xlabel("Inspections")
    left.set_ylabel("Precision")
    percent_axis(left)
    left.set_title("Every extra inspection is worth less")
    right.set_xscale("log")
    right.set_xlabel("Inspections")
    right.set_ylabel("Recall")
    percent_axis(right)
    right.set_title("and catches proportionally more")
    right.legend(fontsize=8)
    caption(
        fig,
        "The operations question is not 'which model is best' but 'how many "
