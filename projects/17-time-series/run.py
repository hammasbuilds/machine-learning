...
def main() -> None:
    ...
    for v in verdicts:
        print(
            f"  {v.series:<26}{v.adf_p:>9.4f}{v.kpss_p:>9.4f}   "
    ...
    if disagreements:
        ...
        print(
            "    'not stationary' is a defensible conclusion, and the analyst picks one anyway.\n"
        )
    ...
    for basis in ("levels", "differences"):
        ...
        print(
            f"  {basis:<16}{row['significant']:>10.1%}{row['median_r2']:>12.3f}"
    ...
    backtest = rolling_backtest(log_price, models, horizon=HORIZON, folds=FOLDS, minimum=240)
    scores = (
    ...
    for name, row in scores.iterrows():
        ...
        print(
            f"  {name:<26}{row['mase']:>8.3f}{row['median_mase']:>9.3f}{row['bias']:>9.3f}"
            f"   {delta:+7.1%}  {verdict}"
        )

    winner = scores.index[0]
    beaten = [
        n for n in scores.index if scores.loc[n, "mase"] < naive and n != "Random walk (last value)"
    ]
    print(
        f"\n    best: {winner}. "
    ...
    fig.suptitle(
        "Everything downstream depends on which of these you model",
        fontsize=12,
        fontweight="bold",
        x=0.02,
        ha="left",
    ...
    caption(
        fig,
        f"Shiller real S&P 500, {frame['Date'].min():%Y}-{frame['Date'].max():%Y}, "
    ...
    left.set_xlabel("|t-statistic| on the slope")
    left.set_ylabel("Trials")
    left.set_title(f"{levels_rate:.0%} of unrelated pairs look significant")
    left.legend(fontsize=8)
    right.set_xlabel("R-squared")
    right.set_ylabel("Trials")
    right.set_title("and the fit looks convincing too")
    right.legend(fontsize=8)
    fig.suptitle(
        "300 pairs of independent random walks. No relationship exists in any of them.",
        fontsize=12,
        fontweight="bold",
        x=0.02,
        ha="left",
    ...
    caption(
        fig,
        "Granger & Newbold (1974). Regressing non-stationary series in levels "
    ...
    colours = [
        PALETTE["green"] if n == "Random walk (last value)" else PALETTE["blue"]
    ...
    dcolours = [
        PALETTE["green"] if n == "Random walk (last value)" else PALETTE["orange"]
    ...
    caption(
        fig,
        f"Expanding window, {HORIZON}-month horizon, minimum 240 months of history. "
    ...
    for name, colour in (
        ("Random walk (last value)", PALETTE["green"]),
        ("ARIMA(2, 1, 2)", PALETTE["blue"]),
        ("Drift", PALETTE["red"]),
    ):
        rows = backtest[backtest["model"] == name]
        ax.plot(
            dates[rows["origin"].to_numpy()],
            rows["mase"],
            "o-",
            lw=1.5,
            ms=3,
    ...
    ax.set_ylabel(f"MASE at a {HORIZON}-month horizon")
    ax.set_yscale("log")
    ax.legend(fontsize=8)
    ax.set_title("The mean hides the variance: any model wins on some folds")
    caption(
        fig,
        "Which model 'wins' depends on which decade you evaluate. Reporting the "
        ...
        json.dumps(
            {
                ...
                "stationarity": [
                    {
                        "series": v.series,
                        "adf_p": round(v.adf_p, 5),
                        "kpss_p": round(v.kpss_p, 5),
                        "agree": v.agree,
                        "verdict": v.verdict,
                    }
                    for v in verdicts
