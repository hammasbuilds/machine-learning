...
def fit_and_score(
    ...
    return (
        float(roc_auc_score(train["y"], model.predict_proba(scaler.transform(train[columns]))[:, 1])),
...
def main() -> None:
    ...
    print(f"  temporal split: {len(train):,} train / {len(test):,} test at "
    ...
    for _, row in profile.iterrows():
        print(f"  {row['appearances']:<14}{row['categories']:>13,}{row['rows']:>10,}"
    ...
    print(f"  {'(no StockCode at all)':<26}{baseline_train:>11.4f}{baseline_test:>10.4f}"
          f"{baseline_train - baseline_test:>8.4f}{'-':>13}")

    rows = []
    for encoder in ENCODERS:
        train["te_stock"], test["te_stock"] = encoder.fit_transform(
            train["stock"].to_numpy(), train["y"].to_numpy(),
            test["stock"].to_numpy(), test["y"].to_numpy(),
        )
        a, b = fit_and_score(train, test, [*NUMERIC, "te_stock"])
        rows.append({"encoding": encoder.name, "train_auc": a, "test_auc": b,
                     "gap": a - b, "lift": b - baseline_test})
    ...
    for encoder in ENCODERS:
        train["te_noise"], test["te_noise"] = encoder.fit_transform(
            train["noise_id"].to_numpy(), train["y"].to_numpy(),
            test["noise_id"].to_numpy(), test["y"].to_numpy(),
        )
    ...
    print(f"    It carries no information. Every point above {baseline_train:.4f} is copied labels.\n")
    ...
    for (low, high), label in zip(buckets, labels, strict=True):
        ...
        leak_rows.append({"appearances": label, "naive": naive_corr, "out_of_fold": oof_corr,
                          "rows": int(mask.sum())})
    ...
    left.set_ylabel("Distinct stock codes"); left.set_xlabel("Times the code appears")
    ...
    right.set_ylabel("Share of all rows"); right.set_xlabel("Times the code appears")
    percent_axis(right)
    right.set_title("but the rare ones are a small slice of the data")
    fig.suptitle("Why one-hot fails here, and why the alternative is dangerous",
                 fontsize=12, fontweight="bold", x=0.02, ha="left")
    caption(fig, "A category seen once has a 'category mean' equal to its own target. That is "
    ...
    ax.bar(x - 0.2, [r["train_auc"] for r in rows], 0.38, label="train AUC",
           color=PALETTE["red"])
    ax.bar(x + 0.2, [r["test_auc"] for r in rows], 0.38, label="test AUC (the truth)",
           color=PALETTE["blue"])
    ax.axhline(baseline_test, color="#333333", ls="--", lw=1.5)
    ax.text(len(rows) - 0.55, baseline_test + 0.008, "no StockCode at all", fontsize=8,
            ha="right", color="#333333")
    for i, r in enumerate(rows):
        ax.text(i - 0.2, r["train_auc"] + 0.006, f"{r['train_auc']:.3f}", ha="center",
    ...
    caption(fig, f"Online Retail II, temporal split, {frame['y'].mean():.2%} return rate. The "
    ...
    ax.bar(x - 0.2, [r["train_auc"] for r in noise_rows], 0.38, label="train AUC",
           color=PALETTE["red"])
    ax.bar(x + 0.2, [r["test_auc"] for r in noise_rows], 0.38, label="test AUC",
           color=PALETTE["blue"])
    ...
    caption(fig, "The column is generated by a random number generator and has no relationship "
    ...
    ax.bar(x - 0.2, [r["naive"] for r in leak_rows], 0.38, label="Naive encoding",
           color=PALETTE["red"])
    ax.bar(x + 0.2, [r["out_of_fold"] for r in leak_rows], 0.38, label="Out-of-fold",
           color=PALETTE["blue"])
    ...
    caption(fig, "Within each rarity bucket, how strongly the encoded feature correlates with "
        ...
        json.dumps(
            {
                ...
                "baseline": {"train_auc": round(baseline_train, 4),
                             "test_auc": round(baseline_test, 4)},
                "stock_code": [{k: (round(v, 4) if isinstance(v, float) else v)
                                for k, v in r.items()} for r in rows],
                "noise_control": [{k: (round(v, 4) if isinstance(v, float) else v)
                                   for k, v in r.items()} for r in noise_rows],
                "leak_by_rarity": [{k: (round(v, 4) if isinstance(v, float) else v)
                                    for k, v in r.items()} for r in leak_rows],
            },
            indent=2,
        )
    )
